...
internal data class NumericSemanticVersion(
    ...
    override fun compareTo(other: NumericSemanticVersion): Int =
        compareValuesBy(this, other, NumericSemanticVersion::major, NumericSemanticVersion::minor, NumericSemanticVersion::patch)
    ...
    companion object {
        ...
        fun parse(value: String): NumericSemanticVersion? {
            ...
            return NumericSemanticVersion(numbers[0], numbers[1], numbers[2])
        }
    }
}
...
internal object GitHubReleaseParser {
    ...
    fun parse(body: String): ReleaseCheckResult {
        val json = runCatching { JSONObject(body) }.getOrNull() ?: return ReleaseCheckResult.NetworkFailure
        if (json.optBoolean("draft") || json.optBoolean("prerelease")) return ReleaseCheckResult.NoRelease
    ...
    fun compareWithInstalled(body: String, installedVersion: String): ReleaseCheckResult {
...
internal class GitHubReleaseUpdateChecker {
    ...
    suspend fun check(installedVersion: String): ReleaseCheckResult = mutex.withLock {
        withContext(Dispatchers.IO) {
            val connection = runCatching { URL(latestReleaseUrl).openConnection() as HttpURLConnection }
            ...
            try {
                ...
                when (connection.responseCode) {
                    HttpURLConnection.HTTP_OK -> {
                        ...
                        GitHubReleaseParser.compareWithInstalled(body, installedVersion)
                    }
