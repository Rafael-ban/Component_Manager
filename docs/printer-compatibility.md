...
当前开发实现提供服务信息诊断、M1 SPP 型号/状态查询、单独确认的测试标签，
以及 Android M1 日常元件与批量打印中心。操作见 [日常打印功能矩阵](m1-printing-feature-matrix.md)。
用户已验证型号与状态查询；Dev.6 已在该台 M1 的 40×60 mm 间隙纸上通过连续两张固定测试图验收。
日常标签、小批量和中断恢复仍需本轮开发版实机验收；其他品牌尚未开放直接打印。
Dev.3 改为在当前诊断页面内复用打印连接（空闲 60 秒后释放），并在发送后查询型号。
当前实现按汉码实机 HCI 的 1 KiB 分帧预算发送完整图像，并分离异步处理通知；
请按 [Dev.6 复测步骤](releases/0.7.4-dev.6.md) 保留第一次和第二次的完整报告。
2026-09-25 已在 Android 15 上取得汉码两次正常出纸与元件仓库短走纸第二次异常的
对照证据。随后 Dev.6 正常模式两张均由用户确认完整、位置正常、无额外空白纸，
第二次复用连接。其他尺寸、固件、长批量及二维码扫描解码仍待验证，不能把通知计数解释为完整出纸。
