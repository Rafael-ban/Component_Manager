...
> 本文保留实施前的发现与优先级，作为决策记录。Android、Windows 和管理台
> 后续完成情况、测试证据与未验收边界见
> [UI consistency implementation report](ui-consistency-implementation.md)。
> 下文“尚未实施”和阶段 B/C 的表述描述审查当时状态，不代表当前代码状态。
...
阶段状态以
[实施报告](ui-consistency-implementation.md) 为准。本次不增加蓝牙打印范围。
Windows 高 DPI、Android 真机/截图和完整辅助技术验收仍不能用编译通过代替。
