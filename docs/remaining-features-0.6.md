...
## 0.7.0 候选实现进度（2026-09-22）

本轮软件功能已合入 `master` 的 `bc5b937`，首轮原生编译修复已由 `7b4e887` 推送。第二轮 Android、服务端、Web 和容器 CI 通过；Windows BOM 测试还有一处 C# 目标类型推断问题，已修复，待重新验证。以下状态不等同于正式发布。
...
- [ ] 完成 Windows 修复后的 CI，并确认最终提交所有检查成功。
...
- [ ] 更新版本与更新日志，以 GitHub CI 和发布产物启动检查成功为 0.7.0 发布依据；真实设备的网络、蓝牙及触摸体验另行记录。
...
| Windows 内外网回退 | 0.7.0 候选实现已补：旧配置安全读取，每次同步先探测主地址，只在 DNS、连接或超时失败时回退；选定地址后整次 push/pull 固定使用，写入中断不换地址重放 |
| BOM 自动匹配、手动选择库存、列映射和缺料 CSV | 0.5.4 已有 CSV/XLSX、工作表选择、确认扣料和本地防重复；0.7.0 候选实现补齐 Android、Windows 任意列映射与缺料 CSV |
...
| 可选 Web 库存操作 | 0.7.0 候选实现已补：默认只读，可通过开关启用元件新增/编辑、入出库与库位创建；使用 CAS、持久化请求回执和短事务 |
| Web 响应式操作与容器部署 | 0.7.0 候选实现已补：手机/桌面界面、独立 Web 镜像及 Compose `web` profile；按需展开表单的新布局浏览器复验通过 |

## 仍未实现或仍需验收

| 项目 | 当前情况 | 后续动作 |
| --- | --- | --- |
| 0.7.0 CI 与发布验收 | Android、服务端、Web、容器检查通过，Windows 测试中的目标类型推断修复待复验 | 确认最终提交 CI 和便携 ZIP 启动成功；真实 Windows 网络环境后续实测 |
