...
public sealed class InventoryWorkbookBackup
{
    ...
    private static int Int(object? v)=>v switch { int i=>i,long l when l is>=int.MinValue and<=int.MaxValue=>(int)l,double d when d==Math.Truncate(d)&&d is>=int.MinValue and<=int.MaxValue=>(int)d,_=>int.Parse(Text(v),NumberStyles.None,CultureInfo.InvariantCulture)};
...
internal static partial class SimpleXlsx
{
    ...
    {
        WriteCore(path, sheets);
        if (images is { Count: > 0 }) AppendImages(path, sheets.Keys.ToArray(), images);
    }
    ...
    private static XDocument Load(ZipArchive z,string p){var e=z.GetEntry(p.Replace("xl/xl/","xl/"))??throw new InvalidDataException("XLSX 缺少 "+p);if(e.Length>20*1024*1024)throw new InvalidDataException("XLSX XML 条目超过限制。");using var s=e.Open();return XDocument.Load(s);}
    private static List<string> Shared(ZipArchive z){var e=z.GetEntry("xl/sharedStrings.xml");if(e is null)return[];using var s=e.Open();return XDocument.Load(s).Descendants(N+"si").Select(x=>string.Concat(x.Descendants(N+"t").Select(t=>t.Value))).ToList();}
    private static int Col(string r){var n=0;foreach(var c in r.TakeWhile(char.IsLetter))n=n*26+char.ToUpperInvariant(c)-'A'+1;return n-1;}private static string Name(int i){var s="";for(i++;i>0;i=(i-1)/26)s=(char)('A'+(i-1)%26)+s;return s;}
}
