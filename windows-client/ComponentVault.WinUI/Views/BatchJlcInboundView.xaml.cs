...
public sealed partial class BatchJlcInboundView:Page
{
    ...
    private void OnBack(object sender,RoutedEventArgs e){if(_busy)return;if(((App)Application.Current).Window is MainWindow window)window.GoBack();}
    public bool TryPrepareToLeave(){if(_busy)return false;PersistEdits();return true;}
    ...
    private async Task ParseAsync(IEnumerable<BatchScanItem> scans)
    {
        ...
        foreach(var scan in scans){if(token.IsCancellationRequested||generation!=_generation)break;Remove(scan.ReceiptId);var parsed=JlcQrParser.Parse(scan.Raw);var editedSku=LcscPublicCatalog.NormalizeSku(scan.Sku);var sku=editedSku??parsed.Sku;var skuChanged=editedSku is not null&&!editedSku.Equals(parsed.Sku,StringComparison.OrdinalIgnoreCase);var editedQuantity=int.TryParse(scan.QuantityText,out var manualQuantity)&&manualQuantity>0?manualQuantity:(int?)null;var row=new BatchInboundEditableRow{ReceiptId=scan.ReceiptId,Raw=scan.Raw,Selected=scan.Selected,Sku=sku,QuantityInput=editedQuantity??parsed.Quantity??double.NaN,LocationId=string.IsNullOrWhiteSpace(scan.LocationId)?defaultLocation:scan.LocationId,Name=skuChanged?"":scan.Name.Length>0?scan.Name:parsed.PartName??parsed.Model??"",Category=skuChanged?"":scan.Category,PackageName=skuChanged?"":scan.PackageName.Length>0?scan.PackageName:parsed.PackageDetail??"",Description=skuChanged?"":scan.Description,Reason=parsed.ParseError??scan.Error,LocationIds=locations};
            ...
            try{if(!cache.TryGetValue(row.Sku,out var lookup)){lookup=await _lookup.LookupDetailedAsync(row.Sku,cancellationToken:token);cache[row.Sku]=lookup;}if(generation!=_generation)break;var metadata=lookup.Metadata;if(metadata is null){row.Reason=string.IsNullOrWhiteSpace(lookup.UserMessage)?"商城无精确匹配，请手动补齐资料。":lookup.UserMessage;_pending.Add(row);continue;}row.Name=metadata.Name;row.Category=metadata.Category;row.PackageName=metadata.PackageName??"";row.Description=CatalogNotes(metadata);if(Complete(row)){row.Reason="";_ready.Add(row);}else{row.Reason=$"{lookup.UserMessage} 商城资料不完整，请手动补齐名称、分类和封装。";_pending.Add(row);}}
    ...
    private static string CatalogNotes(LcscProductMetadata m){var lines=new List<string>();void Add(string? x){if(!string.IsNullOrWhiteSpace(x))lines.Add(x);}Add(m.LookupNotice);Add(m.Model is null?null:$"型号：{m.Model}");Add(m.Description is null?null:$"官方描述：{m.Description}");Add(m.Brand is null?null:$"品牌：{m.Brand}");Add($"官方商品页：{m.OfficialUrl}");Add(m.CategoryPath is null?null:$"官方分类路径：{m.CategoryPath}");Add(m.ImageUrl is null?null:$"商品图片：{m.ImageUrl}");Add(m.DatasheetUrl is null?null:$"数据手册：{m.DatasheetUrl}");if(m.Parameters is not null)foreach(var p in m.Parameters)Add($"参数·{p.Key}：{p.Value}");return string.Join("\n",lines);}
    ...
    private void SetBusy(bool value){_busy=value;HeaderBackButton.IsEnabled=!value;CollectButton.IsEnabled=!value;ParseButton.IsEnabled=!value;ClearButton.IsEnabled=!value;ApplyLocationButton.IsEnabled=!value;SubmitButton.IsEnabled=!value;RetryButton.IsEnabled=!value;RemoveButton.IsEnabled=!value;CaptureBox.IsEnabled=!value;ReadyList.IsEnabled=!value;PendingList.IsEnabled=!value;DefaultLocationBox.IsEnabled=!value;}
