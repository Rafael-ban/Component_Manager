...
from .admin.ui import register_admin_ui
...
@app.post("/auth/ping", response_model=SyncTokenStatus, dependencies=[Depends(require_token)])
def auth_ping() -> SyncTokenStatus:
    return SyncTokenStatus(status="ok", server_time=_utc_now())


@app.post("/sync/push", response_model=PushResponse, dependencies=[Depends(require_token)])
def sync_push(
    payload: PushRequest,
...
@app.get("/sync/pull", response_model=PullResponse, dependencies=[Depends(require_token)])
def sync_pull(
